...
public final class ChatImageUtil {
	 ...
	 * @return
	 */
	public ChatImageUtil appendText(final String... text) {
	 ...
	 * @return
	 */
	public ChatImageUtil appendCenteredText(final String... text) {
		for (int y = 0; y < lines.length; y++)
			if (text.length > y) {
				final int len = ChatPaginator.AVERAGE_CHAT_PAGE_WIDTH - lines[y].length() / lines.length;

				lines[y] = lines[y] + center(text[y], Math.abs(len));
			} else
				return this;
	 ...
	 * @throws IOException
	 */
	public static ChatImageUtil fromFile(@NonNull final File file, final int height, final Type characterType) throws IOException {
		...
		final ChatImageUtil chatImage = new ChatImageUtil();
	 ...
	 * @return
	 */
	public static ChatImageUtil fromLines(final String[] lines) {
		final ChatImageUtil chatImage = new ChatImageUtil();
	...
	private static CompChatColor[][] parseImage(final BufferedImage image, final int height) {
		...
		for (int x = 0; x < resized.getWidth(); x++)
			for (int y = 0; y < resized.getHeight(); y++) {
				...
				final CompChatColor closest = CompChatColor.of(new Color(rgb, true));
